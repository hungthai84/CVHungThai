                        ...
                        categories.map((category) => (
                            <div key={category.key} className="resume-item-card">
                                <h3 className="category-title flex items-center gap-2.5" style={{ color: category.color || themeColor, whiteSpace: 'nowrap', paddingBottom: '0px' }}>
                                    {category.key === 'professional' && <Icons.WrenchScrewdriverIcon className="flex-shrink-0" size={20} />}
                                    {category.key === 'soft' && <Icons.UsersIcon className="flex-shrink-0" size={20} />}
                                    {category.key === 'interdisciplinary' && <Icons.LayersIcon className="flex-shrink-0" size={20} />}
                                    {category.key === 'business' && <Icons.BriefcaseIcon className="flex-shrink-0" size={20} />}
                                    {category.key === 'language' && <Icons.GlobeAltIcon className="flex-shrink-0" size={20} />}
