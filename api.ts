...
// Gemini TTS Endpoint
apiApp.post("/tts/gemini", async (req, res) => {
  try {
...
apiApp.post("/users/me", requireAuth, async (req: AuthRequest, res) => {
  try {
...
apiApp.get("/entries", requireAuth, async (req: AuthRequest, res) => {
  try {
    const uid = req.user.uid;
    const email = req.user.email || "";
    const user = await getOrCreateUser(uid, email);
...
apiApp.post("/entries", requireAuth, async (req: AuthRequest, res) => {
  try {
    const uid = req.user.uid;
    const email = req.user.email || "";
...
apiApp.get("/chat-history", requireAuth, async (req: AuthRequest, res) => {
  try {
    const uid = req.user.uid;
    const email = req.user.email || "";
    const user = await getOrCreateUser(uid, email);
...
apiApp.post("/chat-history", requireAuth, async (req: AuthRequest, res) => {
  try {
    const uid = req.user.uid;
    const email = req.user.email || "";
